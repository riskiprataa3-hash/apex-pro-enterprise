...
// =========================================================================
// SCRIPT IMPORT INLET (VERSI ACAK FOTO OTOMATIS)
// -------------------------------------------------------------------------
...
// namun tetap konsisten pada masing-masing kategori (0%, 50%, 100%).
// =========================================================================
...
// Nama Folder Storage Firebase tempat di uploadnya dokumentasi
const FOLDER_0 = 'dokumentasi 0%';   // Catatan: Jika tidak ketemu, coba ganti angka 0 dengan huruf O 
const FOLDER_50 = 'dokumentasi 50%'; // Misal: 'dokumentasi 5O%'
const FOLDER_100 = 'dokumentasi 100%';

// Daftar Titik KM yang mau Anda set di sistem pencatatan
const DAFTAR_KM = [
  "22+400", "22+405", "22+410", "22+415", "22+420", // Lanjutkan sesuai kemauan Anda
];
...
async function fetchUrls(folderName: string) {
    ...
    try {
        ...
        console.error(`Peringatan: Gagal menemukan/membaca isi dari folder '${folderName}'. Pastikan nama foldernya sama persis.`);
        return [];
    }
}
...
async function run() {
    ...
    if (urls0.length === 0 && urls50.length === 0 && urls100.length === 0) {
        console.error("\n❌ GAGAL: Tidak satupun foto ditemukan dari ketiga folder. Pastikan nama foldernya benar.");
        process.exit(1);
    }
    ...
    console.log(`\n⏳ Mulai mendistribusikan & mensinkronisasi data ke ${DAFTAR_KM.length} Titik KM...`);

    // LOOPING KE MASING-MASING KM & MENGAMBIL FOTO SECARA ACAK
    for (let i = 0; i < DAFTAR_KM.length; i++) {
        const km = DAFTAR_KM[i];
        
        // FUNGSI PENGACAKAN: Jika foto tersedia > 0, ambil satu secara Math.random() 
        ...
        // 1. FORMAT APLIKASI (Sub-collection)
        ...
        // 2. FORMAT EXPORT PDF / DATA RAW KASAR (Koleksi Root)
        ...
        batch.set(rawEntryRef, {
            ...
            ukuran: "37x24",
            jumlah: 1, 
            foto_0: rnd0, // Foto diambil acak yang sama dengan langkah di atasnya
            foto_50: rnd50,
            foto_100: rnd100,
    ...
    console.log(`✅ SUKSES! Sinkronisasi Titik KM Selesai dengan Distribusi Acak Foto Terbaru!`);
    process.exit(0);
}

run().catch(err => {
    console.error("❌ Terjad Kesalahan Script:", err);
    process.exit(1);
});
