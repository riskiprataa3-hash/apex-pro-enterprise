...
async function run() {
    ...
    if (!pId) {
        console.log('Project dummy created');
        pId = doc(collection(db, 'projects')).id;
    }

    let batch = writeBatch(db);
    let opCount = 0;
    
    // Set timestamp ke 18 Mei 2026 jam 12:00:00 (timestamp UTC+7) => ms
    const baseDate = new Date("2026-05-18T12:00:00+07:00");
