...
const rawStrings = [
  ...
  "08+100 A/OS",
  "09+300 A/OS",
  "Akses Keluar Kandis Selatan A/OS",
  "44+000 A/OS",
  "44+005 A/OS",
  "44+015 A/OS",
  "44+020 A/OS",
...
async function runImport() {
    ...
    for (const rawStr of rawStrings) {
        let arah = "A/OS";
        if (rawStr.includes("B/OS")) arah = "B/OS";
        let km = rawStr.replace("A/OS", "").replace("B/OS", "").trim();

        // Include A/OS or B/OS back to km string
    ...
    await batch.commit();
    console.log(`SUKSES: ${count} entri data berhasil dimasukkan untuk tanggal ${TANGGAL_TEKS}!`);
    process.exit(0);
}
